...
public:
    ...
    if(working_){
      ...
      new_octree_result_cloud = true;
      {
        ...
        if (new_octree_result_cloud && new_track_model_cloud && new_input_cloud &&
          ...
          ros::ServiceClient client = n_->serviceClient<aginika_pcl_ros::SetTrackerObjectId>("renew_model");
          aginika_pcl_ros::SetTrackerObjectId srv;
  ...
  bool recieve_signal(aginika_pcl_ros::SetWorkingStateAndTopic::Request &req,
                      aginika_pcl_ros::SetWorkingStateAndTopic::Response &response
                      )
  {
    if(req.state.data){
