...
#include "aginika_pcl_ros/SetPointCloud2.h"
#include "aginika_pcl_ros/SetWorkingState.h"
#include "aginika_pcl_ros/SetWorkingStateAndTopic.h"
...
public:
  ParticleFilterTrackerManager():INIT_ID(1),SEGMENTATION_MIN_SIZE(20),target_id_(INIT_ID),n_("~"),CAMERA_X_ABS_MAX(0.411428571),CAMERA_Y_ABS_MAX(0.5685715),counter_(0),prev_tracker_mutable_id_(-1){
    ...
    pub_ = n_.advertise<aginika_pcl_ros::TrackerInfoArray>("tracker_info_array", 1);;
  }


  bool set_mainobject_cb(aginika_pcl_ros::SetMainObjectId::Request &req,
                         aginika_pcl_ros::SetMainObjectId::Response &res)
  {
  ...
  bool update_cb(aginika_pcl_ros::SetTrackerObjectId::Request &req,
                 aginika_pcl_ros::SetTrackerObjectId::Response &res)
  {
  ...
  bool exchange_cb(aginika_pcl_ros::SetTrackerObjectId::Request &req,
                   aginika_pcl_ros::SetTrackerObjectId::Response &res)
  {
    ROS_INFO("Get request :Multi_tracking_upgrade_manager");
    ROS_INFO("            :tracker_id %d object_id %s", int(req.tracker_id), req.new_object_id.c_str());
  ...
  void send_pointcloud_to_tracker_service(int target_id, const sensor_msgs::PointCloud2& cloud, TrackingObjectInfo::TrackingObjectProperty mutable_model = TrackingObjectInfo::UNKNOWN){
    ...
    ros::ServiceClient client = n_.serviceClient<aginika_pcl_ros::SetPointCloud2>(tracker_name_+boost::lexical_cast<std::string>(target_id)+tracker_service_name_);
    aginika_pcl_ros::SetPointCloud2 srv;
  ...
  void send_boot_mutable_model(int tracker_id, bool bool_wake)
  {
    ...
    ros::ServiceClient client = n_.serviceClient<aginika_pcl_ros::SetWorkingStateAndTopic>(simple_mutable_model_name_+"/reset_state");
    aginika_pcl_ros::SetWorkingStateAndTopic ws;
  ...
  void send_boot_tracker_service(int tracker_id, bool bool_wake, std::string prev_object_id)
  {
    ...
    ros::ServiceClient client = n_.serviceClient<aginika_pcl_ros::SetWorkingState>(tracker_name_+boost::lexical_cast<std::string>(tracker_id)+"/reset_state");
    aginika_pcl_ros::SetWorkingState ws;
    ws.request.state.data = bool_wake;
  ...
  void publishTrackerInfo(){
    aginika_pcl_ros::TrackerInfoArray tia;
    typedef std::pair<int,TrackingObjectInfo::Ptr> P;
    BOOST_FOREACH(P toii, tracker_and_object_info_){
      aginika_pcl_ros::TrackerInfo ti;
